            ...
            >
              ...
              <div className="p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 rounded-xl border border-gray-100 bg-gray-50/70 p-4 text-sm">
                      ...
                      {!selectedPayment.is_full_payment && (
                        <>
                          <div className="flex justify-between"><span className="text-gray-500">EMI Number:</span> <span className="font-medium text-gray-900">{selectedPayment.emi_number || 1} of {selectedPayment.total_emis || totalEmis}</span></div>
                ...
                {!selectedPayment.is_full_payment && selectedPayment.emi_duedate && (
                  <div className="mt-6 grid md:grid-cols-2 gap-6">
                    <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4 shadow-sm">
                      ...
                      <p className="text-sm text-indigo-600">EMI Number: {(selectedPayment.emi_number || 1) + 1} of {selectedPayment.total_emis || totalEmis}</p>
                ...
                <div className="mt-6 flex flex-wrap justify-end gap-3 border-t border-gray-100 pt-4">
                  <button onClick={() => downloadReceipt(selectedPayment._id)} className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700">Download Receipt (PDF)</button>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <div className="flex justify-center gap-2" onClick={e => e.stopPropagation()}>
                            <button onClick={() => downloadReceipt(payment._id)} className="rounded-lg bg-blue-50 p-1.5 text-blue-600 transition hover:bg-blue-100" title="Download Receipt"><FaFileInvoiceDollar size={14} /></button>
            ...
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div><label className="mb-1 block text-xs font-bold uppercase tracking-wider text-gray-500">Payment Date</label>
                      <input type="date" name="payment_date" value={formData.payment_date} onChange={e => setFormData({ ...formData, payment_date: e.target.value })}
                        className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" /></div>
                    <div><label className="mb-1 block text-xs font-bold uppercase tracking-wider text-gray-500">Receipt ID (optional)</label>
                      <input type="text" name="receipt" value={formData.receipt} onChange={e => setFormData({ ...formData, receipt: e.target.value })}
                        className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" placeholder="Optional" /></div>
                    <div className="sm:col-span-2"><label className="mb-1 block text-xs font-bold uppercase tracking-wider text-gray-500">Transaction ID (for manual payments)</label>
                      <input type="text" name="txn_id" value={formData.txn_id} onChange={e => setFormData({ ...formData, txn_id: e.target.value })}
                        className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none transition-all focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" placeholder="For offline payments" /></div>
                  </div>
                  ...
                  {selectedStudent && (
                    <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-3 text-sm text-gray-700">
